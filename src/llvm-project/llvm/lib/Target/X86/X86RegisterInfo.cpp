...
BitVector X86RegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  const Function* F = &MF.getFunction();
//  if(MF.getTarget().Options.MCOptions.ATSGX){
    if(MF.getTarget().Options.MCOptions.ATSGX && ATSGX_SAFE != ATSGX_isLeaky<Function>(F)){
    //ATSGX: reserve RAX (rax may use)
    Reserved.set(X86::RAX);
    Reserved.set(X86::EAX);
    Reserved.set(X86::AX);
    Reserved.set(X86::AL);

    //ATSGX: Reserve R15 for springboard
