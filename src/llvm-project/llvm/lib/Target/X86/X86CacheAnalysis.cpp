...
void X86CacheAnalysis::doInitialize(MachineFunction &MF, AsmPrinter &AP) {
  ...
  //ATSGX
  if (!revert_live_out_info.empty())
    revert_live_out_info.clear();

  while(!blk_live_out_recoder.empty()){
    Rendezvous * _tmp = blk_live_out_recoder.back();
    free(_tmp);
    blk_live_out_recoder.pop_back();
  }
...
void X86CacheAnalysis::processInstruction(const MachineInstr *MI) {
  ...
  if(op_code == X86::DBG_VALUE || op_code == X86::CFI_INSTRUCTION){
    return;
  }
...
//ATSGX
bool X86CacheAnalysis::checkAllInLoop(struct X86CacheAnalysis::Rendezvous * rendezvous, const class MachineLoop * ML) {
  //exclude blks in sub-loops
  for(auto blk: ML->blocks()){
    MachineLoop *machineLoop = LI->getLoopFor(blk);
    if(machineLoop != ML)
      continue;
    auto itr = rendezvous->basic_blocks.find(blk);
    if(itr==rendezvous->basic_blocks.end()){
      return false;
    }
  }
  return true;
...
void X86CacheAnalysis::Rendezvous::clear_invalid_blks() {
  std::set<const MachineBasicBlock*> rm_working_set;
  std::map<const MachineBasicBlock *,float>::iterator itr;

  //lambda functin for checking successor
  auto _handler_working_set = [this, &itr, &rm_working_set](){
    float live_out_value = itr->second;
    const MachineBasicBlock * bb = itr->first;
    //1. 是否超出了threshold 的blks
    if(live_out_value > threshold){
      rm_working_set.insert(bb);
    }

    //2. 如果subcessors 都在聚合圈外，且不是泄露基本块
    if(!is_subcessor_existed(bb) && !ATSGX_MBB_isLeaky(bb)){
      rm_working_set.insert(bb);
    }
  };

  errs()<<__func__<<":::";
  //遍历当前聚合区的基本块，将多余的添加到rm_woring_set
  // 1. 将live out value 超过threshold 的添加到rm_woring_set
  // 2. subcessor 都在聚合圈外，且不是基本块的
  for(itr=blk_cache_num_live_out.begin();itr!=blk_cache_num_live_out.end();itr++){
    _handler_working_set();
  }

  do{
    while (!rm_working_set.empty()){
      const MachineBasicBlock * mbb = *rm_working_set.begin();
      rm_working_set.erase(rm_working_set.begin());

      for(const MachineBasicBlock* succ: mbb->successors()){
        //1. check whether this succ existed in this rendezvous
        itr = blk_cache_num_live_out.find(succ);
        if(itr == blk_cache_num_live_out.end()){
          continue;
        }
        //2. 是否超出了threshold
        _handler_working_set();
      ...
      errs()<< "remove MBB_"<<mbb->getNumber()<<" ";
      remove_blk(mbb);
    }
...
void X86CacheAnalysis::_analyzeMBBCacheLiveOut(
    struct llvm::X86CacheAnalysis::Rendezvous * rendezvous,
    const class llvm::MachineBasicBlock * MBB, std::set<const MachineBasicBlock*> &working_set){

  //threshold & coefficient value
  float threshold = rendezvous->threshold;
  float penalty_coef =  ATSGX_MBB_isLeaky(MBB)== ATSGX_SAFE ? rendezvous->penalty_coef : 1.0;

  //当前基本块的循环信息
  const MachineLoop *cur_loop = LI->getLoopFor(MBB);
  bool is_loop_header = LI->isLoopHeader(MBB);

  float cur_blk_live_out_value;
  if(MBB->pred_begin() == MBB->pred_end()){
    cur_blk_live_out_value = (float)getBBCacheWayUsage(MBB->getNumber());
  }else{
    //1. 计算 prev bblks 到 cur blk 的 live out value
    std::vector<float> live_outs;
    for(MachineBasicBlock * prev_bb: MBB->predecessors()){
      //获取前驱节点的live out value
      float prev_bb_live_out_value;
      bool is_prev_bb_in_rendezvous = rendezvous->get_blk_live_out(prev_bb, prev_bb_live_out_value);

      //MBB为loop的header，且prev_bb和MBB在同一个loop中；
      //如果当前loop的bb都在聚合圈（不算sub-loop），我们设置prev_bb的live_out为0.0， 避免循环计算，同时相当于将entry更新到了 loop header
      const MachineLoop *target_loop = LI->getLoopFor(prev_bb);
      if(target_loop){
        const MachineBasicBlock * target_loop_header = target_loop->getHeader();
        if(target_loop_header == MBB){
          if(checkAllInLoop(rendezvous, target_loop)){
            prev_bb_live_out_value = 0.0;
          }
        }
      }

      //计算当前路径的live out 值
      float path_live_out_value;
      if(!is_prev_bb_in_rendezvous){
        //path_live_out_value = cur_cache_way_usage
        path_live_out_value = (float)getBBCacheWayUsage(MBB->getNumber());
      }else{
        //path_live_out_value = pre_bb_live_out + ((PREV_BB, CUR_BB) - PREV_BB)*penalty_coef;
        path_live_out_value = prev_bb_live_out_value
                              + penalty_coef * (float)(getJointBBCacheWayUsage(prev_bb->getNumber(),MBB->getNumber())
                                                       - getBBCacheWayUsage(prev_bb->getNumber()));
      }

      live_outs.push_back(path_live_out_value);
    }

    //2. get the maximun value of the live outs as the cur blk's live_out
    cur_blk_live_out_value = *std::max_element(live_outs.begin(), live_outs.end());
  }


  //3.1 live out value > threshold，且MBB不在聚集区，直接结束
  if(cur_blk_live_out_value > threshold && rendezvous->is_mbb_existed(MBB)){
    return;
  }

  //3.2 基本块的live out value 进行更新
  RendezvousState state = rendezvous->update(MBB, cur_blk_live_out_value);

  //状态没有变化，结束处理
  if(state==UNCHANGED) return;

  //3.3 当前的基本块的live out值有了更新，对successors进一步分析
  for(MachineBasicBlock *target_bb: MBB->successors()){
    //0. 如果successor 之前已经被添加到聚合区中，则强制更新，这样方便在清理阶段将超出的blk清理掉
    if(rendezvous->is_mbb_existed(target_bb)){
      working_set.insert(target_bb);
      continue;
    }

    //1. target_bb 是 cur_loop_header, cur_bb 为 latch
    // 且循环的所有基本块都在聚合区中
    // 则直接将target_bb添加到working_set 中
    if(cur_loop){
      MachineBasicBlock *cur_loop_header = cur_loop->getHeader();
      if(cur_loop_header && target_bb == cur_loop_header){
          if(checkAllInLoop(rendezvous,cur_loop)){
            working_set.insert(target_bb);
            continue;
          }
      }
    ...
    //2. 计算successor 的cache，如果超出threshold, 就不添加到working_set中
    float target_penalty_coef =  ATSGX_MBB_isLeaky(target_bb)== ATSGX_SAFE ? rendezvous->penalty_coef : 1.0;
    float target_path_live_out_value = cur_blk_live_out_value \
        + target_penalty_coef * (float)(getJointBBCacheWayUsage(MBB->getNumber(), target_bb->getNumber()) \
        - getBBCacheWayUsage(MBB->getNumber()));

    if(target_path_live_out_value > threshold)
      continue;

    const MachineLoop *target_loop = LI->getLoopFor(target_bb);
    //target_bb 是一个新的loop的entry
    if(target_loop){
      MachineBasicBlock *target_header = target_loop->getHeader();
      if((target_header ) && (target_loop!=cur_loop))
        continue;
    }

    //target_bb 是当前loop的end
    // check if the current basicblock is a loop header as
    // a loop always ends in a loop header.
    if(is_loop_header && (!target_loop||(target_loop != cur_loop))){
        continue;
    }
...
void X86CacheAnalysis::analyzeMBBCacheLiveOut(const MachineBasicBlock *MBB) {
  //1. 如果当前MBB已经被分配到一个聚合圈中，则无需再进行处理
  auto itr = revert_live_out_info.find(MBB);
  if(itr != revert_live_out_info.end() && itr->second != nullptr){
    //MBB已经被分配到聚集圈中，无需进一步处理
    return;
  }

  //以当前MBB为基点，构建聚合圈
  //2. 以当前MBB为entry，分析和创建聚合圈
  errs()<<__func__<<" MBB:"<<MBB->getNumber()<<"\n";
  Rendezvous *rendezvous = new Rendezvous(MBB, blk_live_out_recoder.size());
//  rendezvous->basic_blocks.insert(MBB);

  std::set<const MachineBasicBlock*> working_set;
  working_set.insert(MBB);
  //3. 直到set中所有的基本块被处理完，达到一个平衡点
  while(working_set.size()!=0){
    const MachineBasicBlock * mbb = *working_set.begin();
    working_set.erase(working_set.begin());
    _analyzeMBBCacheLiveOut(rendezvous, mbb, working_set);
  }

  //4. 对聚合圈的基本块进行清理
  rendezvous->clear_invalid_blks();

  //5. 构建聚合圈和聚合圈的映射
  blk_live_out_recoder.push_back(rendezvous);
  for(const MachineBasicBlock *bb: make_range(rendezvous->basic_blocks.begin(), rendezvous->basic_blocks.end())){
    revert_live_out_info.insert(std::make_pair(bb, rendezvous));
  }

  return;
}

void X86CacheAnalysis::analyzeCacheLiveOut() {
  //ATSGX errs()
  //MF->print(errs());
  //对每个存在泄露的基本块进行分析处理
  for(auto &mbb: *MF){
    if(ATSGX_MBB_isLeaky(&mbb) != ATSGX_SAFE){
      analyzeMBBCacheLiveOut(&mbb);
    }
  }
}
