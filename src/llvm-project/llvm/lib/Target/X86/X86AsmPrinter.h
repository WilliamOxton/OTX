...
class LLVM_LIBRARY_VISIBILITY X86AsmPrinter : public AsmPrinter {
  ...
  //ATSGX
  bool ATSGXHandleInstr(X86MCInstLower &MCInstLowering, const MachineInstr *MI);

  //ATSGX: UNSAFE function
  bool ATSGXHandleUnsafeFuncInstr(X86MCInstLower &MCInstLowering, const MachineInstr *MI);
  void ATSGXHandleUnsafeMBBFirstInstr(const Function *F,const MachineBasicBlock *MBB,
                                      const BasicBlock *B, const MachineInstr *MI);
  bool ATSGXHandleUnsafeMBBBranchInstr(const MachineBasicBlock *MBB, const MachineInstr *MI);
  bool ATSGXHandleUnsafeMBBCallInstr(X86MCInstLower &MCInstLowering, const MachineBasicBlock *MBB, const MachineInstr *MI);
  bool ATSGXHandleUnsafeMBBRetInstr(const MachineBasicBlock *MBB,
                                   const BasicBlock *B, const MachineInstr *MI);

  //ATSGX: Leaky function
  bool ATSGXHandleLeakyFuncInstr(X86MCInstLower &MCInstLowering, const MachineInstr *MI);
  void ATSGXHandleMBBFirstInstr(const MachineBasicBlock *MBB, const MachineInstr *MI);
  bool ATSGXHandleMBBBranchInstr(X86MCInstLower &MCInstLowering, const MachineBasicBlock *MBB, const MachineInstr *MI);
  bool ATSGXHandleMBBCallInstr(X86MCInstLower &MCInstLowering, const MachineBasicBlock *MBB, const MachineInstr *MI);

  bool ATSGXHandleMBBRetInstr(X86MCInstLower &MCInstLowering, const MachineBasicBlock *MBB, const MachineInstr *MI);

  void ATSGXSaveRAX(bool insert);
  void ATSGXRestoreRAX(bool insert);
  bool ATSGXGenSpringboardBetweenBBs(const MachineBasicBlock *SrcBB, const MachineBasicBlock *DestBB,
                                     const MachineInstr *MI, int OptLevel);

  bool _ATSGXGenSpringboardBetweenBBs(const MachineBasicBlock *SrcBB,
  ...
  bool _ATSGXGenSpringboardBetweenBBs(const MachineBasicBlock *SrcBB,
                                      ...
                                      const llvm::MachineInstr *MI);
  void ATSGXJumpSpringBoard(MCSymbol *target_sym, const Twine &Springboard_name, const MachineInstr *MI);
  void ATSGXDirJumpSpringBoard(MCSymbol *target_sym, const Twine &Springboard_name);

  MCSymbol * ATSGXGetUnUsedSymbol(const Twine &prefix, const Twine &Suffix, int &id);
