...
static void readConfigs(opt::InputArgList &args) {
  ...
  //ATSGX
  config->ATSGX = args.hasFlag(OPT_ATSGX, OPT_no_ATSGX, false);
  config->ATSGXDebug = args.hasFlag(OPT_ATSGXDebug, OPT_no_ATSGXDebug, false);
  config->ATSGXOpt = args::getInteger(args, OPT_ATSGXOpt, 0);
  config->leakageInfoFilePath = args.getLastArgValue(OPT_leakage_info_file);
  if (config->ATSGX == true)
    config->outputFile = saver.save(args.getLastArgValue(OPT_o).str() + ".atsgx");
  else
